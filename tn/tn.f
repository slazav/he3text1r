...
C                FAIRFAX, VA 22030
C******************************************************************
      SUBROUTINE TN (IERROR, N, X, F, G, W, LW, SFUN, MSGLVL)
...
C           ( 4 => CAN NOT FIND DECENT DIRECTION
C           (-1 => ERROR IN INPUT PARAMETERS
C N       - (INTEGER) NUMBER OF VARIABLES
C X       - (REAL*8) VECTOR OF LENGTH AT LEAST N; ON INPUT, AN INITIAL
C           ESTIMATE OF THE SOLUTION; ON OUTPUT, THE COMPUTED SOLUTION.
...
C
     ...
     *            MSGLVL, MAXIT, MAXFUN, ETA, STEPMX, ACCRCY, XTOL)
      IMPLICIT          DOUBLE PRECISION (A-H,O-Z)
      INTEGER           MSGLVL, N, MAXFUN, IFAIL, LW, IPIVOT(N)
...
C
      DOUBLE PRECISION DABS, DDOTX, DSQRT, STEP1, DNRM2X
      EXTERNAL SFUN
...
20    CONTINUE
      CALL DCOPYX(N,G,1,W(LOLDG),1)
      PNORM = DNRM2X(N,W(LPK),1)
      OLDF = FNEW
      OLDGTP = GTPNEW
C
...
C
      CALL DCOPYX(N,W(LSK),1,W(LSR),1)
      CALL DCOPYX(N,W(LYK),1,W(LYR),1)
      FKEEP = FNEW
      ICYCLE = 1
      GO TO 20
C
...
C
      DOUBLE PRECISION DABS, DDOTX, DNRM2X, DSQRT, STEP1
      EXTERNAL SFUN
      COMMON/SUBSCR/ LGV, LZ1, LZK, LV, LSK, LYK, LDIAGB, LSR, LYR,
     *     LOLDG, LHG, LHYK, LPK, LEMAT, LWTEST
C
...
C
      ...
      IF (DIFNEW .LT. 5.0D-1*DIFOLD) EPSRED = 5.0D-1*EPSRED
60    CALL DCOPYX(N,G,1,W(LGV),1)
      CALL ZTIME(N,W(LGV),IPIVOT)
      GTG = DDOTX(N,W(LGV),1,W(LGV),1)
      GNORM = DSQRT(GTG)
      FTEST = ONE + DABS(FNEW)
      XNORM = DNRM2X(N,X,1)
C
...
C ...............END OF MAIN ITERATION.......................
C
 130  if (GTG .LT. PEPS*FTEST*FTEST) goto 135
      IFAIL = 4
      goto 136
...
c         print '(A,I5,3G17.8)','spe2:',i,t,p(i),spe
         GO TO 10
5        T = UP(I) - X(I)
         IF (T .LT. SPE*P(I)) SPE = T / P(I)
c         print '(A,I5,3G17.8)','spe3:',i,t,p(i),spe
10    CONTINUE
      RETURN
      END
C
...
C         TOL = DSQRT(EPSMCH) * (DABS(LOW(I)) + 1.D0)
...
15    CONTINUE
      CONV = .FALSE.
      ONE = 1.D0
      if (GTG .GE. 1.D-4*ACCRCY*FTEST*FTEST .AND.
     *     (ALPHA*PNORM .GE. TOLEPS*(ONE + XNORM)
     *        .OR. DABS(DIFNEW) .GE. RTLEPS*FTEST)) return
      CONV = .TRUE.
C
...
C
         CALL DAXPYX(N,ALPHA,V,1,ZSOL,1)
         CALL DAXPYX(N,-ALPHA,GV,1,R,1)
C
C TEST FOR CONVERGENCE
C
         GTP = DDOTX(N,ZSOL,1,G,1)
         PR = DDOTX(N,R,1,ZSOL,1)
         QNEW = 5.D-1 * (GTP + PR)
...
C
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION E(N),V(N),GV(N),R(N),VGV,VR,DDOTX
C
...
C      MCHPR1 = 1.D-17
C for me      
      MCHPR1 = 0.222044604925031308d-15
C
C FOR SUN
C
...
C
      DOUBLE PRECISION DDOTX,YKSK,GSK,YRSR,RDIAGB,YKHYK,GHYK,
...
57    CONTINUE
      IF (FIRST) YKSR = DDOTX(N,YK,1,SR,1)
      IF (FIRST) YKHYR = DDOTX(N,YK,1,HYR,1)
      GSR = DDOTX(N,G,1,SR,1)
      GHYR = DDOTX(N,G,1,HYR,1)
      IF (FIRST) YRHYR = DDOTX(N,YR,1,HYR,1)
      CALL SSBFGS(N,ONE,SR,YR,HG,HYR,YRSR,
     *     YRHYR,GSR,GHYR,HG)
      IF (FIRST) CALL SSBFGS(N,ONE,SR,YR,HYK,HYR,YRSR,
     *     YRHYR,YKSR,YKHYR,HYK)
      YKHYK = DDOTX(N,HYK,1,YK,1)
      GHYK = DDOTX(N,HYK,1,G,1)
...
C
      DO 65 I = 1,N
         ...
         IF (FIRST) HYK(I) = YK(I)*RDIAGB
65    CONTINUE
      IF (FIRST) YKHYK = DDOTX(N,YK,1,HYK,1)
      GHYK = DDOTX(N,G,1,HYK,1)
      CALL SSBFGS(N,ONE,SK,YK,HG,HYK,YKSK,
     *     YKHYK,GSK,GHYK,Y)
      RETURN
100   CONTINUE
...
C
     ...
     *     YK(N),COND,SR(N),YR(N),DDOTX,SDS,SRDS,YRSK,TD,D1,DN
      LOGICAL LRESET,UPD1
      IF (UPD1) GO TO 90
      IF (LRESET) GO TO 60
      DO 10 I = 1,N
         BSK(I) = DIAGB(I)*SR(I)
10    CONTINUE
      SDS = DDOTX(N,SR,1,BSK,1)
      SRDS = DDOTX(N,SK,1,BSK,1)
      YRSK = DDOTX(N,YR,1,SK,1)
      DO 20 I = 1,N
         TD = DIAGB(I)
         BSK(I) = TD*SK(I) - BSK(I)*SRDS/SDS+YR(I)*YRSK/YRSR
         EMAT(I) = TD-TD*TD*SR(I)*SR(I)/SDS+YR(I)*YR(I)/YRSR
20    CONTINUE
      SDS = DDOTX(N,SK,1,BSK,1)
      DO 30 I = 1,N
